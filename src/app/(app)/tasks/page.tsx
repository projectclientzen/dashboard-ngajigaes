...
import { MOCK_TASK_COMMENTS } from '@/lib/mock'
...
export default function TasksPage() {
  ...
  const comments = selectedId ? MOCK_TASK_COMMENTS.filter(c => c.task_id === selectedId) : []
  ...
  const segBtn = (active: boolean) =>
    cn('px-[14px] py-[6px] rounded-md text-[12px] font-semibold cursor-pointer border-none transition-all',
      ...
      {view === 'kanban' && (
        <div className="flex gap-3 overflow-x-auto pb-[10px]">
          {KANBAN_COLS.map(col => {
            ...
            return (
              <div key={col.id}
                className="w-[232px] flex-shrink-0 bg-[#F0EBDA] rounded-xl p-[10px]"
                onDragOver={e => e.preventDefault()}
                onDrop={() => { if (dragId) { updateStatus(dragId, col.id); setDragId(null) } }}>
                {/* col header */}
                    ...
                    <div key={tk.id}
                      ...
                      <div className="flex items-center gap-[7px] pt-2 border-t border-[#F1ECDC]">
                        <div className="w-[22px] h-[22px] rounded-md flex items-center justify-center text-[9px] font-semibold text-white"
                          style={{ background: avatarColor(tk.assignee_name) }}>
      ...
      {selectedId && selected && (
        <div className="fixed inset-0 z-50">
          ...
          <div className="absolute top-0 right-0 w-[430px] max-w-[92vw] h-full bg-[#FBF8EE] shadow-drawer p-[22px] overflow-y-auto animate-drawer-in flex flex-col gap-4">
            {/* header */}
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-semibold text-[#8A8267] bg-[#EFEAD9] rounded-full px-[10px] py-[3px]">
                {selected.category}
              </span>
              <button onClick={() => setSelectedId(null)}
                className="bg-none border-none cursor-pointer text-[#9A9279] text-[20px] leading-none">×</button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-[14px] p-[14px] bg-[#FCFAF2] border border-[#EFE9D8] rounded-lg">
              <div>
                ...
                <div className="flex items-center gap-[7px]">
                  <div className="w-6 h-6 rounded-md flex items-center justify-center text-[9px] font-semibold text-white"
                    style={{ background: avatarColor(selected.assignee_name) }}>
              ...
              <div>
                ...
                <div className="text-[13px] font-semibold text-[#2B2A24]">
                  {selected.deadline ? formatDate(selected.deadline, 'd MMM yyyy') : 'Tanpa deadline'}
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="text-[13px] text-[#5A574C] leading-[1.55]">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => { updateStatus(selected.id, 'done'); setSelectedId(null) }}
                  ...
                  <div key={c.id} className="flex gap-[9px]">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-[7px]">
                        ...
                        <span className="text-[10px] text-[#B0A78C]">{formatDate(c.created_at, 'd MMM, HH:mm')}</span>
                      </div>
