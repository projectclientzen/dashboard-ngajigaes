...
export function Header() {
  ...
  const { userRole, userName, dateRange, setDateRange } = useApp()
  const [title, subtitle] = PAGE_TITLES[pathname] ?? ['Dashboard', '']

  const rangeBtn = (r: '7d' | '30d' | '90d') =>
    cn(
      'px-[11px] py-[5px] rounded-md text-[12px] font-semibold cursor-pointer transition-all border',
      dateRange === r
      ...
      <div className="flex items-center gap-[14px]">
        <div className="flex items-center gap-[6px]">
          ...
          {(['7d','30d','90d'] as const).map(r => (
            <button key={r} className={rangeBtn(r)} onClick={() => setDateRange(r)}>
